...
type LoginProps = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'StudentSelection'>;
};
  ...
  const callLoginApi = async () => {
    ...
    try {
      ...
      if (res && res.status === 200 && res.data) {
        ...
        if (selected === 'student') {
          if (rememberMe) {
            await StorageManager.saveLoginDataStudent(
              studentId.trim(),
              password.trim(),
              rememberMe,
            );
          ...
          if (rememberMe) {
            await StorageManager.saveLoginDataParents(
              studentId.trim(),
              password.trim(),
              rememberMe,
            );
